def filtering_category(database,
                       category_key = None,
                       ordering_key = None,
                       reverse = False):
    """
    ...
    """
    if category_key:
        result = [p for p in database.values() if p.get('category') == category_key]  # TODO При помощи фильтрации в list comprehension профильтруйте товары по категории (ключ 'category') в продукте database. Или можете использовать

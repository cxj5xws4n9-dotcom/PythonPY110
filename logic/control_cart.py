...
def add_to_cart(id_product: str, username: str = '') -> bool:
    ...
    if id_product not in DATABASE:
        return False# TODO Проверьте существует ли добавляемый товар с id_product в базе данных DATABASE, если нет, то возвращаем False,
    ...
    if id_product in user_cart:
        ...
        user_cart[id_product] = 1# TODO Проверьте, а существует ли товар с id_product в корзине пользователя user_cart, если товар существует,
    ...
    with open(PATH_CART, 'w', encoding = 'utf-8') as f:
        json.dump(cart, f)# TODO Не забываем записать обновленные данные cart в 'cart.json'. Так как именно из этого файла мы считываем данные
...
def remove_from_cart(id_product: str, username: str = '') -> bool:
    ...
    if id_product not in user_cart:
        return False# TODO Проверьте, существует ли товар с id_product в корзине пользователя user_cart, если нет, то возвращаем False.

    del user_cart[id_product]# TODO Если существует товар, то удаляем ключ 'id_product' у user_cart,
    #  вспомните как удалять ключи у словаря.

    with open(PATH_CART, 'w', encoding='utf-8') as f:
        json.dump(cart, f)# TODO Не забываем записать обновленные данные cart в 'cart.json', аналогично как делали в add_to_cart
